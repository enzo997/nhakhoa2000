<?php
    ...
    function fresh_numbers_style() {
        ...
        wp_enqueue_style('home-page', THEME_URL.'/assets/css/home-page.css?'.$date);
        wp_enqueue_style('header', THEME_URL.'/assets/css/header.css?'.$date);
...
//////end
////
function create_new_custom_post_type_Service(){

    $args = array(
        "post_type" => 'Service',
        "name" => "Service",//show menu left
        "single" => "Service",
        "slug" => "service",//link ten bai post
        'menu_icon' => 'dashicons-star-empty',
    );
    nt_create_post_type($args);
}
add_action('init', 'create_new_custom_post_type_service');


//taxaonomies chua nhieu cac post-list
function create_custom_taxonomies_service() {
    ...
    $args = array(
        ...
        "name" => "Categories",
        "single" => "categories",
        "slug" => "categories_service",
        "taxonomy" => "categories_service",
    );
    nt_create_taxonomy($args);
}
add_action('init', 'create_custom_taxonomies_service', 0);
